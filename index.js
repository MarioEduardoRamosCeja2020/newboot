...
client.on('message', async msg => {
  ...
  const now = Date.now();
  const sender = chat.isGroup ? chat.participants.find(p => p.id._serialized === senderId) : { isAdmin: true };
  ...
  try {
    ...
    if (msg.hasMedia) {
      ...
      if (!media.mimetype.startsWith('image/')) {
        return chat.sendMessage('⚠️ Solo se pueden generar stickers a partir de imágenes.');
      }
...
`);
    ...
    if (command === '.todos' && chat.isGroup) {
      const last = groupCooldowns[chat.id._serialized]?.todos || 0;
      const wait = COOLDOWNS.todos - (now - last);
      if (!isAdmin) return chat.sendMessage('⚠️ Solo administradores pueden usar este comando.');
      if (wait > 0) return chat.sendMessage(`⏳ Por favor espera ${Math.ceil(wait/1000)} segundos antes de usar .todos de nuevo.`);
      ...
      logEvent('ACTION', '.todos ejecutado', { group: chat.name });
      return;
    }

    // .hidetag
    if (command === '.hidetag' && chat.isGroup) {
      const last = groupCooldowns[chat.id._serialized]?.hidetag || 0;
      const wait = COOLDOWNS.hidetag - (now - last);
      if (!isAdmin) return chat.sendMessage('⚠️ Solo los admins pueden usar este comando.');
      if (wait > 0) return chat.sendMessage(`⏳ Por favor espera ${Math.ceil(wait/1000)} segundos antes de usar .hidetag de nuevo.`);
    ...
    if (command === '.notify' && chat.isGroup) {
      const last = groupCooldowns[chat.id._serialized]?.notify || 0;
      const wait = COOLDOWNS.notify - (now - last);
      if (!isAdmin) return chat.sendMessage('⚠️ Solo los admins pueden usar este comando.');
      if (wait > 0) return chat.sendMessage(`⏳ Por favor espera ${Math.ceil(wait/1000)} segundos antes de usar .notify de nuevo.`);
    ...
    // Aquí puedes agregar tus otras funciones: mesas, formarpareja, memes, imagenes, musica
    // Solo asegúrate de manejar errores con mensajes suaves
  } catch (err) {
    logEvent('ERROR', 'Error general', { error: err.message });
    await chat.sendMessage('⚠️ Ocurrió un error, intenta de nuevo.');
  }
});
